...
//                accessFile.seek(accessFile.getFilePointer() - 1); /// chert......
    ...
    public static LexicalAnalyzer initializeLexical() {
        ...
        for (int i = 1; i <= 17; i++) {
            ...
            switch (i){
                case 2:
                    isfinal = true;
                    needToBack = true;
                    tokenType = TokenType.IDENTIFIYER;
                    break;
                case 4:
                    isfinal = true;
                    needToBack = true;
        ...
        map.put(new Transition(start,InputType.EQUAL),states[5]);
        ...
        //state5
        map.put(new Transition(states[5],InputType.EQUAL),states[6]);

        for (InputType value : InputType.values()) {
            if(value != InputType.EQUAL && value != InputType.OTHER ){
        ...
        //state13
        map.put(new Transition(states[13],InputType.ENDLINE),states[13]);
        for (InputType value : InputType.values()) {
            if(value != InputType.ENDLINE && value != InputType.EOF){
                map.put(new Transition(states[13],value),states[13]);
            }
        }

        //state15
        map.put(new Transition(states[15],InputType.ENDLINE),states[15]);
        map.put(new Transition(states[15],InputType.WHITESPACE),states[15]);

        for (InputType value : InputType.values()) {
            if(value != InputType.ENDLINE && value != InputType.WHITESPACE){
