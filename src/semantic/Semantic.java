...
public class Semantic {
    ...
    private static SymbolTable curSymbolTable = new SymbolTable();
    private static Stack semanticStack = new Stack();

    public void addSemanticRoutine(Edge edge, String semanticCheck, Token curToken) {
        String varName = curToken.getToken();
        for (String semanticMark : semanticCheck.replaceAll("$", " ").split("\\s")) {
            SemanticTokenType tokenType = SemanticTokenType.getSemanticToken(semanticMark);
            if (tokenType == null) {
                System.out.println(semanticMark + " token not found!");
                return;
            }
            switch (tokenType) {
                case NOT_VOID:
    ...
    private void incrementVariableAddress() {
        variableAddress += 4;
    }

    private void incrementTempAddress() {
        tempAddress += 4;
    }
